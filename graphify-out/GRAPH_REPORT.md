# Graph Report - svelteforge-u7-317  (2026-09-10)

## Corpus Check
- 378 files · ~462,983 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2903 nodes · 3624 edges · 299 communities (139 shown, 103 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 7 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9f0658a1`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- en.json
- fr.json
- design-system.ts
- devDependencies
- svforge/package.json
- email/package.json
- graph/package.json
- release-plan.mjs
- dnd/package.json
- oauth/package.json
- tiptap/package.json
- uploads/package.json
- ui_toast/package.json
- blog/package.json
- jobs/package.json
- audit/package.json
- chat/package.json
- notifications/package.json
- realtime/package.json
- svforge-check.mjs
- compilerOptions
- primitives/types.ts
- RealtimeHub
- $lib/paraglide/messages.js
- SVForge
- $lib/utils/cn
- AGENTS.md — SvelteForge
- svforge/src/upgrade.ts
- base.ts
- helpers.ts
- scripts
- capabilities.ts
- svforge/src/index.ts
- svforge.test.ts
- @svforge/realtime
- prebuild-utils.ts
- devDependencies
- chat/index.ts
- compilerOptions
- SVForge
- better-auth-audit.mjs
- properties
- users/+page.svelte
- auth.schema.ts
- release-plan.schema.json
- registry
- @svforge/audit
- jobs/index.ts
- @svforge/notifications
- addon-kit/src/upgrade.ts
- users/+page.server.ts
- $app/state
- devDependencies
- changelog
- @svforge/jobs
- createRealtimeClient
- scripts
- settings.json
- What You Get (in addition to Base)
- @svforge/tiptap
- npm release process
- required
- @svforge/chat
- @svforge/email
- svforge/scripts/prebuild.ts
- doctor.ts
- Contributing to SVForge
- @svforge/blog
- posts.ts
- reset-password.ts
- @svforge/oauth
- PRD — SvelteForge Monorepo
- @svforge/uploads
- Security Policy
- properties
- base/src/lib/components/svforge/ui/index.ts
- schemas.ts
- manifest-freshness.test.ts
- gen-modules-table.mjs
- npm-consumer-smoke.mjs
- admin-feedback.test.ts
- bun.ts
- json.ts
- audit/index.ts
- addon-kit/package.json
- check-auth-schema.mjs
- @svforge/graph
- Versioning Strategy
- SvelteForge Base Template
- structural-duplication.ts
- skeleton-markup.test.ts
- addon-kit/src/index.ts
- @svforge/dnd
- Button.svelte
- better-auth-upgrade.mjs
- scripts
- notifications/index.ts
- package.json
- @svforge/ui_toast
- Better Auth upgrades — policy & automation (#319)
- upload/upload-security.test.ts
- canary-issue.mjs
- web-hardening.test.ts
- file-upload-contract.test.ts
- svforge@1.2.0 — 2026-09-06
- @svforge/graph@0.0.1 — 2026-09-06
- @svforge/jobs@0.0.1 — 2026-09-06
- @svforge/notifications@0.0.1 — 2026-09-06
- @svforge/oauth@0.0.1 — 2026-09-06
- @svforge/realtime@0.0.1 — 2026-09-06
- @svforge/tiptap@0.0.2 — 2026-09-06
- @svforge/ui_toast@0.0.2 — 2026-09-06
- @svforge/uploads@0.0.1 — 2026-09-06
- @svforge/audit@0.0.1 — 2026-09-06
- @svforge/blog@0.0.1 — 2026-09-06
- @svforge/chat@0.0.1 — 2026-09-06
- @svforge/dnd@0.0.2 — 2026-09-06
- @svforge/email@0.0.1 — 2026-09-06
- localDependencies
- post-form.ts
- @svforge/addon-kit@0.0.1 — 2026-09-06
- @svforge/addon-kit
- base/package.json
- blog-mdsvex.test.ts
- check-generated.test.ts
- email-security.test.ts
- ghost-classes.test.ts
- paraglide-keys.test.ts
- repository-metadata.test.ts
- skeleton-inventory.ts
- tiptap-render.test.ts
- uuid-id-contracts.test.ts
- TDD Workflow — Red → Green → Refactor
- Contribution Workflow
- Toaster.svelte
- Modules
- admin-auth.test.ts
- audit-pagination.test.ts
- llms-docs-freshness.test.ts
- tiptap-xss.test.ts
- tests/upload-security.test.ts
- audit/+page.server.ts
- Welcome!
- [id]/+page.server.ts
- fetch-llms-docs.sh
- base/src/hooks.server.ts
- dashboard/src/app.d.ts
- settings/+page.server.ts
- login/+page.server.ts
- Templates
- a11y-reactivity.test.ts
- admin-delete.test.ts
- build-config.test.ts
- followSystemTheme
- theme-v5.test.ts
- Changelog
- dashboard/package.json
- @fontsource-variable/manrope
- tsconfig.test-d.json
- vite-design-system.test.ts
- test-local.ts
- svforge/src/app.d.ts
- eslint
- @eslint/compat
- eslint-config-prettier
- @eslint/js
- @fontsource-variable/fira-code
- @fontsource-variable/inter
- @fontsource-variable/space-grotesk
- globals
- phosphor-svelte
- prettier
- prettier-plugin-svelte
- @skeletonlabs/skeleton
- @skeletonlabs/skeleton-common
- @skeletonlabs/skeleton-svelte
- svelte-check
- @sveltejs/adapter-auto
- @sveltejs/kit
- @sveltejs/vite-plugin-svelte
- tailwind-merge
- tailwindcss
- @tailwindcss/forms
- @tailwindcss/vite
- typescript
- vite
- vitest
- base/src/app.d.ts
- eslint
- @eslint/compat
- environment.ts
- @eslint/js
- svelte-shims.d.ts
- @fontsource-variable/fira-code
- @fontsource-variable/inter
- Experimental structural duplication detector
- phosphor-svelte
- postgres
- prettier
- prettier-plugin-svelte
- prettier-plugin-tailwindcss
- @skeletonlabs/skeleton
- @skeletonlabs/skeleton-common
- @skeletonlabs/skeleton-svelte
- svelte
- svelte-check
- @sveltejs/adapter-auto
- @sveltejs/kit
- tailwind-merge
- tailwindcss
- @tailwindcss/forms
- @tailwindcss/typography
- primitives-ssr.test.ts
- @types/node
- typescript
- vite
- setup.sh
- dashboard/src/hooks.server.ts
- client/auth.ts
- admin-users.test.ts
- types.ts
- setup/+page.server.ts
- test-scaffold.sh
- deterministic-manifest.test.ts
- scaffold-paths.test.ts
- security-policy.test.ts
- eslint
- eslint-plugin-svelte
- globals
- @skeletonlabs/skeleton
- @skeletonlabs/skeleton-svelte
- @testing-library/jest-dom
- @testing-library/svelte
- @types/jsdom
- @types/ws
- typescript
- typescript-eslint
- vitest
- ws
- clsx
- @sveltejs/vite-plugin-svelte
- typescript-eslint

## God Nodes (most connected - your core abstractions)
1. `path` - 20 edges
2. `compilerOptions` - 18 edges
3. `planAddonContext()` - 17 edges
4. `applyBaseMode()` - 17 edges
5. `SVForge` - 17 edges
6. `checkModuleCapabilities()` - 16 edges
7. `ROOT` - 16 edges
8. `Changelog` - 16 edges
9. `applyPlan()` - 15 edges
10. `checkDesignSystem()` - 15 edges

## Surprising Connections (you probably didn't know these)
- `installBaseline()` --calls--> `resolveDestination()`  [EXTRACTED]
  tests/upgrade-engine.test.ts → packages/addon-kit/src/upgrade.ts
- `installBaseline()` --calls--> `initTrackingJson()`  [EXTRACTED]
  tests/upgrade-engine.test.ts → packages/addon-kit/src/upgrade.ts
- `buildAddonComponents()` --calls--> `readDirRecursively()`  [EXTRACTED]
  packages/svforge/scripts/generate-skeleton-inventory.ts → scripts/prebuild-utils.ts
- `scaffoldDashboard()` --calls--> `applyBaseMode()`  [EXTRACTED]
  tests/bun-run-check.test.ts → packages/svforge/src/modes/base.ts
- `scaffoldFiles()` --calls--> `applyBaseMode()`  [EXTRACTED]
  tests/eslint-design-system.test.ts → packages/svforge/src/modes/base.ts

## Import Cycles
- None detected.

## Communities (299 total, 103 thin omitted)

### Community 0 - "en.json"
Cohesion: 0.01
Nodes (175): admin_active_sessions, admin_dashboard, admin_manage_users, admin_pending, admin_recent_users, admin_this_week, admin_title, admin_total_users (+167 more)

### Community 1 - "fr.json"
Cohesion: 0.01
Nodes (175): admin_active_sessions, admin_dashboard, admin_manage_users, admin_pending, admin_recent_users, admin_this_week, admin_title, admin_total_users (+167 more)

### Community 2 - "design-system.ts"
Cohesion: 0.08
Nodes (32): ADDON_COMPONENTS, ARBITRARY_SPACING_NAMESPACES, AvoidPattern, AvoidPatternSpec, CatalogEntry, checkAvoidPatterns(), checkClassString(), checkDesignSystem() (+24 more)

### Community 3 - "devDependencies"
Cohesion: 0.15
Nodes (13): jsdom, devDependencies, @eslint/js, jsdom, sv, svelte, @sveltejs/vite-plugin-svelte, @types/bun (+5 more)

### Community 4 - "svforge/package.json"
Cohesion: 0.04
Nodes (45): author, bin, svforge, bugs, description, devDependencies, sv, @svforge/addon-kit (+37 more)

### Community 5 - "email/package.json"
Cohesion: 0.05
Nodes (39): author, description, devDependencies, sv, @svforge/addon-kit, tsdown, typescript, exports (+31 more)

### Community 6 - "graph/package.json"
Cohesion: 0.05
Nodes (40): author, description, devDependencies, sv, @svforge/addon-kit, tsdown, typescript, exports (+32 more)

### Community 7 - "release-plan.mjs"
Cohesion: 0.09
Nodes (33): compareVersions(), entriesBetween(), FIELD_HEADINGS, parseAttributes(), parseChangelog(), readChangelog(), ROOT, validateChangelog() (+25 more)

### Community 8 - "dnd/package.json"
Cohesion: 0.05
Nodes (39): author, description, devDependencies, sv, @svforge/addon-kit, tsdown, typescript, exports (+31 more)

### Community 9 - "oauth/package.json"
Cohesion: 0.05
Nodes (39): author, description, devDependencies, sv, @svforge/addon-kit, tsdown, typescript, exports (+31 more)

### Community 10 - "tiptap/package.json"
Cohesion: 0.05
Nodes (39): author, description, devDependencies, sv, @svforge/addon-kit, tsdown, typescript, exports (+31 more)

### Community 11 - "uploads/package.json"
Cohesion: 0.05
Nodes (39): author, description, devDependencies, sv, @svforge/addon-kit, tsdown, typescript, exports (+31 more)

### Community 12 - "ui_toast/package.json"
Cohesion: 0.05
Nodes (38): author, description, devDependencies, sv, @svforge/addon-kit, tsdown, typescript, exports (+30 more)

### Community 13 - "blog/package.json"
Cohesion: 0.05
Nodes (37): author, description, devDependencies, sv, @svforge/addon-kit, tsdown, typescript, exports (+29 more)

### Community 14 - "jobs/package.json"
Cohesion: 0.05
Nodes (37): author, description, devDependencies, sv, @svforge/addon-kit, tsdown, typescript, exports (+29 more)

### Community 15 - "audit/package.json"
Cohesion: 0.06
Nodes (36): author, description, devDependencies, sv, @svforge/addon-kit, tsdown, typescript, exports (+28 more)

### Community 16 - "chat/package.json"
Cohesion: 0.06
Nodes (36): author, description, devDependencies, sv, @svforge/addon-kit, tsdown, typescript, exports (+28 more)

### Community 17 - "notifications/package.json"
Cohesion: 0.06
Nodes (36): author, description, devDependencies, sv, @svforge/addon-kit, tsdown, typescript, exports (+28 more)

### Community 18 - "realtime/package.json"
Cohesion: 0.06
Nodes (36): author, description, devDependencies, sv, @svforge/addon-kit, tsdown, typescript, exports (+28 more)

### Community 19 - "svforge-check.mjs"
Cohesion: 0.09
Nodes (28): gitignorePath, plugin, rule, ADDON_COMPONENTS, AVOID_PATTERNS, checkDesignSystem(), classViolations(), collectCatalogExempts() (+20 more)

### Community 20 - "compilerOptions"
Cohesion: 0.06
Nodes (34): bun, DOM, ES2023, **/node_modules/**, ./packages/addon-kit/src/index.ts, packages/*/scripts/**/*.ts, packages/*/src/**/*.ts, ./packages/svforge/src/index.ts (+26 more)

### Community 21 - "primitives/types.ts"
Cohesion: 0.06
Nodes (30): AnchorProps, ButtonColor, ButtonOrAnchorProps, ButtonProps, ButtonSharedProps, ButtonSize, ButtonVariant, TableBaseProps (+22 more)

### Community 22 - "RealtimeHub"
Cohesion: 0.12
Nodes (12): createRealtimeHub(), RealtimeClient, RealtimeEvent, RealtimeHub, RealtimeServerOptions, SocketEntry, realtime, delay() (+4 more)

### Community 23 - "$lib/paraglide/messages.js"
Cohesion: 0.14
Nodes (6): ./$types, ./$types, ./$types, ./$types, ./$types, ./$types

### Community 24 - "SVForge"
Cohesion: 0.08
Nodes (21): Deployment profiles, Minimal examples, Node application with attached realtime, Runtime modules, Security and data lifecycle, Serverless web app with a worker, Decision (#349), Evaluation (+13 more)

### Community 25 - "$lib/utils/cn"
Cohesion: 0.15
Nodes (4): describedByIds, describedByIds, describedByIds, options

### Community 26 - "AGENTS.md — SvelteForge"
Cohesion: 0.10
Nodes (19): AGENTS.md — SvelteForge, Architecture, Clôture de roadmap, Conventions composants (templates), Git, ⚠️ LE gotcha du repo : le prebuild n'embarque que `src/**` (+ `root/**` pour le dashboard), Pièges connus (vérifiés), Point d'entrée docs (+11 more)

### Community 27 - "svforge/src/upgrade.ts"
Cohesion: 0.13
Nodes (20): PlannedOperation, UpgradePlan, ChangelogEntry, compareVersions(), entriesBetween(), RELEASE_NOTES, MODULE_RECIPE_DATA, ModuleRecipeData (+12 more)

### Community 28 - "base.ts"
Cohesion: 0.08
Nodes (34): BASE_ROOT_PATHS, DASHBOARD_ROOT_PATHS, LOCAL_DOCS, readLocalDoc(), scaffoldedLocalDocs(), applyBaseMode(), HookMode, applyDashboardMode() (+26 more)

### Community 29 - "helpers.ts"
Cohesion: 0.13
Nodes (15): baseRoot, dashboardRoot, dashboardRoot, baseTemplateFile(), dashboardTemplateFile(), expectFile(), expectNoFile(), packageDir() (+7 more)

### Community 30 - "scripts"
Cohesion: 0.14
Nodes (14): scripts, auth:schema, build, check, check:watch, db:generate, db:migrate, db:push (+6 more)

### Community 31 - "capabilities.ts"
Cohesion: 0.09
Nodes (26): AUTH_HOOKS_SIGNALS, CAPABILITIES, Capability, CAPABILITY_TOKENS, CapabilityEvaluation, CapabilityGate, CapabilityInfo, compositionGaps() (+18 more)

### Community 32 - "svforge/src/index.ts"
Cohesion: 0.22
Nodes (17): JsonGuardError, parseJsonFile(), planManifestEnrichContent(), validateManifestShape(), BASE_PATTERNS, buildManifest(), DASHBOARD_PATTERNS, DEPLOYMENT_PROFILES (+9 more)

### Community 33 - "svforge.test.ts"
Cohesion: 0.30
Nodes (3): ROOT, ROOT, SVFORGE

### Community 34 - "@svforge/realtime"
Cohesion: 0.12
Nodes (15): Architecture, Client — subscribe, Dependencies, Envelope, Install, License, Limits (v1), Option A — adapter-node (customServer) (+7 more)

### Community 35 - "prebuild-utils.ts"
Cohesion: 0.11
Nodes (18): __dirname, __dirname, __dirname, __dirname, __dirname, __dirname, __dirname, __dirname (+10 more)

### Community 36 - "devDependencies"
Cohesion: 0.13
Nodes (15): devDependencies, clsx, eslint-plugin-svelte, prettier-plugin-tailwindcss, svelte, @tailwindcss/typography, @types/node, typescript-eslint (+7 more)

### Community 37 - "chat/index.ts"
Cohesion: 0.24
Nodes (11): A, B, OUTSIDER, assertMember(), chat, getLastMessage(), unreadCount(), conversationParticipants (+3 more)

### Community 38 - "compilerOptions"
Cohesion: 0.14
Nodes (13): compilerOptions, allowJs, checkJs, esModuleInterop, forceConsistentCasingInFileNames, moduleResolution, resolveJsonModule, rewriteRelativeImportExtensions (+5 more)

### Community 39 - "SVForge"
Cohesion: 0.13
Nodes (15): A design system that agents can follow, Agent instruction support (#347), AI-ready, Boring by design, Compose only what the project needs, Defaults vs constraints, Development, Getting started (+7 more)

### Community 40 - "better-auth-audit.mjs"
Cohesion: 0.14
Nodes (22): audit(), BASELINE_PATH, formatFindings(), isBaselined(), isTransientNetworkError(), loadBaseline(), queryOsv(), resolvedPackages() (+14 more)

### Community 41 - "properties"
Cohesion: 0.15
Nodes (13): pattern, type, properties, pattern, type, minLength, type, directory (+5 more)

### Community 42 - "users/+page.svelte"
Cohesion: 0.18
Nodes (3): config, ./$types, config

### Community 43 - "auth.schema.ts"
Cohesion: 0.12
Nodes (12): account, accountRelations, session, sessionRelations, user, userRelations, verification, closeDb (+4 more)

### Community 44 - "release-plan.schema.json"
Cohesion: 0.17
Nodes (11): additionalProperties, $id, packages, required, $schema, title, type, changelog (+3 more)

### Community 45 - "registry"
Cohesion: 0.17
Nodes (12): availableVersions, latest, published, registry, additionalProperties, format, properties, required (+4 more)

### Community 46 - "@svforge/audit"
Cohesion: 0.17
Nodes (11): Admin view, 🔒 Confidentiality — what must NEVER go in `metadata`, Deployment profile, Install, License, Limits (v1), Model, Read (+3 more)

### Community 47 - "jobs/index.ts"
Cohesion: 0.20
Nodes (6): handlers, JobHandler, JobHandlerContext, jobsApi, JobStatus, jobs

### Community 48 - "@svforge/notifications"
Cohesion: 0.15
Nodes (12): Create, Deployment profile, Install, License, Limits (v1), Model, Optional: email (#239/#236), Optional: realtime (#229) (+4 more)

### Community 49 - "addon-kit/src/upgrade.ts"
Cohesion: 0.11
Nodes (36): ApplyOptions, applyPlan(), ApplyResult, assertSafeRelativePath(), BACKUP_ROOT, baselineChecksum(), computeBaseline(), currentRange() (+28 more)

### Community 52 - "devDependencies"
Cohesion: 0.12
Nodes (17): better-auth, drizzle-kit, devDependencies, better-auth, drizzle-kit, drizzle-orm, eslint-config-prettier, eslint-plugin-svelte (+9 more)

### Community 53 - "changelog"
Cohesion: 0.18
Nodes (11): additionalProperties, properties, required, type, minimum, type, const, changelog (+3 more)

### Community 54 - "@svforge/jobs"
Cohesion: 0.18
Nodes (10): Define a handler, Deployment profile, Enqueue, Guarantees (v1) — read before relying on it, Install, License, Optional composition, States (+2 more)

### Community 55 - "createRealtimeClient"
Cohesion: 0.27
Nodes (10): ClientOptions, createRealtimeClient(), close(), connect(), sendSubscribe(), sendUnsubscribe(), subscribe(), unsubscribe() (+2 more)

### Community 56 - "scripts"
Cohesion: 0.18
Nodes (11): scripts, build, check, check:watch, dev, format, lint, prepare (+3 more)

### Community 57 - "settings.json"
Cohesion: 0.18
Nodes (10): baseLocale, locales, modules, plugin.inlang.messageFormat, pathPattern, $schema, en, fr (+2 more)

### Community 58 - "What You Get (in addition to Base)"
Cohesion: 0.17
Nodes (11): Admin Dashboard — golden references (#267), Auth System, Database, Database lifecycle (#332), Environment Variables, Next Steps, Pre-configured Files, Routes Structure (+3 more)

### Community 59 - "@svforge/tiptap"
Cohesion: 0.17
Nodes (11): Dependencies, Deployment profile, Editor (editable), Editor + Preview side by side, Installation, Preview (read-only), Storage, @svforge/tiptap (+3 more)

### Community 60 - "npm release process"
Cohesion: 0.29
Nodes (6): Authentication and permissions, Independent versioning, Local preflight, npm release process, Resuming a failed release, Workflow gates

### Community 61 - "required"
Cohesion: 0.20
Nodes (10): additionalProperties, required, items, type, packages, directory, localDependencies, manifestPath (+2 more)

### Community 62 - "@svforge/chat"
Cohesion: 0.18
Nodes (10): API, Composition (all optional), Deployment profile, Install, License, Model, Security, @svforge/chat (+2 more)

### Community 63 - "@svforge/email"
Cohesion: 0.18
Nodes (10): API, Dependencies, Deployment profile, Installation, `sendEmail(options)`, Setup, @svforge/email, Templates (+2 more)

### Community 64 - "svforge/scripts/prebuild.ts"
Cohesion: 0.11
Nodes (18): buildAddonComponents(), buildSkeletonInventory(), kebabDirToExports(), SkeletonInventory, addonComponents, baseFiles, baseRootFilesRaw, changelogEntries (+10 more)

### Community 65 - "doctor.ts"
Cohesion: 0.06
Nodes (55): DeploymentProfile, boundAllows(), branchAllowsMajor(), branchSatisfies(), Capability, CAPABILITY_ENV_VARS, CAPABILITY_LABELS, caretUpperBound() (+47 more)

### Community 66 - "Contributing to SVForge"
Cohesion: 0.22
Nodes (9): Building, Contributing to SVForge, Development Setup, Project Principles, Running Tests, Test Categories, Test Helpers, What to Avoid (+1 more)

### Community 67 - "@svforge/blog"
Cohesion: 0.20
Nodes (9): Create a Post, Deployment profile, File Structure, Frontmatter Format, Install, License, MDsveX, Routes (+1 more)

### Community 68 - "posts.ts"
Cohesion: 0.28
Nodes (5): getPost(), Post, PostComponent, postFiles, PostMeta

### Community 69 - "reset-password.ts"
Cohesion: 0.47
Nodes (5): resetPasswordEmailHtml(), escapeHtml(), SAFE_PROTOCOLS, sanitizeHref(), welcomeEmailHtml()

### Community 70 - "@svforge/oauth"
Cohesion: 0.20
Nodes (9): 1. Environment Variables, 2. Configure Better Auth, Deployment profile, Install, License, Prerequisites, Setup, @svforge/oauth (+1 more)

### Community 71 - "PRD — SvelteForge Monorepo"
Cohesion: 0.22
Nodes (8): Architecture, CSS foundation, Install, License, Modules, PRD — SvelteForge Monorepo, Summary, Templates

### Community 72 - "@svforge/uploads"
Cohesion: 0.22
Nodes (8): Deployment profile, Environment Variables, Install, License, Size policies, @svforge/uploads, Usage, What it does

### Community 73 - "Security Policy"
Cohesion: 0.22
Nodes (9): Coordinated disclosure, Leaked credentials and secrets, Reporting a vulnerability, Safe harbor, Security Policy, Supported versions, Vulnerabilities affecting generated projects, What a useful report contains (+1 more)

### Community 76 - "properties"
Cohesion: 0.25
Nodes (8): minLength, type, properties, commit, schemaVersion, versionPolicy, const, const

### Community 77 - "base/src/lib/components/svforge/ui/index.ts"
Cohesion: 0.21
Nodes (5): boundedText(), CHANGE_FREQUENCIES, escapeXml(), generateSitemap(), SitemapEntry

### Community 78 - "schemas.ts"
Cohesion: 0.25
Nodes (7): ChangePasswordSchema, CreateUserSchema, LoginSchema, SetupSchema, ToggleUserStatusSchema, ToggleVerifySchema, UpdateUserSchema

### Community 79 - "manifest-freshness.test.ts"
Cohesion: 0.25
Nodes (9): assertNoDrift(), checkGenerated(), discoverPrebuildPackages(), FIX_ADVICE, FIX_COMMAND, run(), SCRIPT_ROOT, modulePackages (+1 more)

### Community 80 - "gen-modules-table.mjs"
Cohesion: 0.25
Nodes (4): __dirname, manifest, REQ_LABEL, ROOT

### Community 81 - "npm-consumer-smoke.mjs"
Cohesion: 0.43
Nodes (6): consumerSource(), consumerTsConfig(), packageTypePath(), runConsumerSmoke(), runTypecheck(), validateInstalledTypes()

### Community 82 - "admin-feedback.test.ts"
Cohesion: 0.25
Nodes (7): LAYOUT, LOGIN_SERVER, ROOT, SETTINGS_SERVER, SETUP_SERVER, USERS_PAGE, USERS_SERVER

### Community 83 - "bun.ts"
Cohesion: 0.29
Nodes (6): root, root, findBun(), runBun(), OAUTH_PKG, ROOT

### Community 84 - "json.ts"
Cohesion: 0.05
Nodes (33): checkModuleCapabilities(), formatCapabilityWarnings(), MODULE_DEPLOYMENT_SUPPORT, ModuleDeploymentSupport, snapshotProject(), CatalogMerge, JsonGuardResult, lineColumnOf() (+25 more)

### Community 85 - "audit/index.ts"
Cohesion: 0.38
Nodes (5): audit, AuditEntryInput, AuditFilters, AuditListOptions, auditLogs

### Community 86 - "addon-kit/package.json"
Cohesion: 0.07
Nodes (26): author, description, devDependencies, tsdown, typescript, exports, files, dist/ (+18 more)

### Community 87 - "check-auth-schema.mjs"
Cohesion: 0.13
Nodes (17): compareSchemas(), deriveRuntimeModel(), matchBrace(), normalizeDefaultLiteral(), normalizeRuntimeSchema(), parseColumnChunk(), parseDrizzleTables(), sameDefault() (+9 more)

### Community 88 - "@svforge/graph"
Cohesion: 0.25
Nodes (7): Deployment profile, Features, Installation, License, Props, @svforge/graph, Usage

### Community 89 - "Versioning Strategy"
Cohesion: 0.29
Nodes (6): CI/CD Channels, Install, Packages, Release Process, Rules, Versioning Strategy

### Community 90 - "SvelteForge Base Template"
Cohesion: 0.20
Nodes (9): Adding a locale (example: Spanish), Components, Defaults vs constraints, Next Steps, Routes, SEO and theme customization, Styles, SvelteForge Base Template (+1 more)

### Community 91 - "structural-duplication.ts"
Cohesion: 0.18
Nodes (18): AstRecord, CachedFingerprint, checkStructuralDuplicates(), clearStructuralFingerprintCache(), Fingerprint, fingerprintCache, fingerprintSvelte(), isAstRecord() (+10 more)

### Community 92 - "skeleton-markup.test.ts"
Cohesion: 0.15
Nodes (7): AVOID_PATTERNS, checkArbitraryTokens(), COMPONENTS, DASHBOARD_TS, ROOT, CTX, ROOT

### Community 93 - "addon-kit/src/index.ts"
Cohesion: 0.11
Nodes (15): files, project(), runAddon(), ADDON_INDEX, blogAddon, { dir: fixtureDir, cleanup: cleanupFixture }, ROOT, runAddon() (+7 more)

### Community 94 - "@svforge/dnd"
Cohesion: 0.29
Nodes (6): Dependencies, Deployment profile, Installation, @svforge/dnd, Usage, What's included

### Community 95 - "Button.svelte"
Cohesion: 0.15
Nodes (9): anchorClasses, anchorRest, buttonRest, classes, inactive, isAnchor, presetClass, sizeClass (+1 more)

### Community 96 - "better-auth-upgrade.mjs"
Cohesion: 0.24
Nodes (11): applyUpgrades(), BLOCKING_SEVERITIES, bumpRangeInSource(), changelogUrl(), classifyBump(), compareVersions(), currentPin(), PIN_FILES (+3 more)

### Community 97 - "scripts"
Cohesion: 0.22
Nodes (9): scripts, audit, build, build:all, lint, release:plan, test, test:watch (+1 more)

### Community 98 - "notifications/index.ts"
Cohesion: 0.40
Nodes (4): NotificationInput, NotificationListOptions, notificationsApi, notifications

### Community 99 - "package.json"
Cohesion: 0.25
Nodes (7): dependencies, packages/*, name, packageManager, private, type, workspaces

### Community 100 - "@svforge/ui_toast"
Cohesion: 0.29
Nodes (6): Dependencies, Deployment profile, Installation, @svforge/ui_toast, Usage, What's included

### Community 101 - "Better Auth upgrades — policy & automation (#319)"
Cohesion: 0.29
Nodes (7): 1.7.x migration notes (applied in #319), Better Auth upgrades — policy & automation (#319), Known follow-ups, Manual run, The CLI (`@better-auth/cli`) is NOT a scaffold dependency, The gate — "tested" is part of the policy, Upgrade policy

### Community 102 - "upload/upload-security.test.ts"
Cohesion: 0.20
Nodes (7): ALLOWED_MIME_TYPES, getSizePolicy(), POST(), sanitizeFilename(), createPresignedPost, env, getSignedUrl

### Community 104 - "web-hardening.test.ts"
Cohesion: 0.29
Nodes (3): compiledSeo, compiledThemeToggle, generatedDir

### Community 105 - "file-upload-contract.test.ts"
Cohesion: 0.33
Nodes (3): COMPILED, FILE_UPLOAD, ROOT

### Community 106 - "svforge@1.2.0 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, svforge@1.2.0 — 2026-09-06

### Community 107 - "@svforge/graph@0.0.1 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, @svforge/graph@0.0.1 — 2026-09-06

### Community 108 - "@svforge/jobs@0.0.1 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, @svforge/jobs@0.0.1 — 2026-09-06

### Community 109 - "@svforge/notifications@0.0.1 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, @svforge/notifications@0.0.1 — 2026-09-06

### Community 110 - "@svforge/oauth@0.0.1 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, @svforge/oauth@0.0.1 — 2026-09-06

### Community 111 - "@svforge/realtime@0.0.1 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, @svforge/realtime@0.0.1 — 2026-09-06

### Community 112 - "@svforge/tiptap@0.0.2 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, @svforge/tiptap@0.0.2 — 2026-09-06

### Community 113 - "@svforge/ui_toast@0.0.2 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, @svforge/ui_toast@0.0.2 — 2026-09-06

### Community 114 - "@svforge/uploads@0.0.1 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, @svforge/uploads@0.0.1 — 2026-09-06

### Community 115 - "@svforge/audit@0.0.1 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, @svforge/audit@0.0.1 — 2026-09-06

### Community 116 - "@svforge/blog@0.0.1 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, @svforge/blog@0.0.1 — 2026-09-06

### Community 117 - "@svforge/chat@0.0.1 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, @svforge/chat@0.0.1 — 2026-09-06

### Community 118 - "@svforge/dnd@0.0.2 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, @svforge/dnd@0.0.2 — 2026-09-06

### Community 119 - "@svforge/email@0.0.1 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, @svforge/email@0.0.1 — 2026-09-06

### Community 120 - "localDependencies"
Cohesion: 0.40
Nodes (5): type, items, type, uniqueItems, localDependencies

### Community 121 - "post-form.ts"
Cohesion: 0.33
Nodes (4): encoder, MAX_FILE_SIZE, MAX_MULTIPART_FORM_OVERHEAD, MAX_POST_BODY_SIZE

### Community 122 - "@svforge/addon-kit@0.0.1 — 2026-09-06"
Cohesion: 0.40
Nodes (5): Breaking changes, Deprecations, Fixes, Migrations, @svforge/addon-kit@0.0.1 — 2026-09-06

### Community 123 - "@svforge/addon-kit"
Cohesion: 0.40
Nodes (4): Capability contract (#323), Safe JSON merges (#324), @svforge/addon-kit, Vocabulary

### Community 124 - "base/package.json"
Cohesion: 0.40
Nodes (4): name, private, type, version

### Community 125 - "blog-mdsvex.test.ts"
Cohesion: 0.40
Nodes (4): ADDON_INDEX, ARTICLE_SERVER, POSTS_UTIL, ROOT

### Community 126 - "check-generated.test.ts"
Cohesion: 0.50
Nodes (3): git(), makeWorkspace(), ROOT

### Community 127 - "email-security.test.ts"
Cohesion: 0.40
Nodes (4): RESET, ROOT, SECURITY, WELCOME

### Community 128 - "ghost-classes.test.ts"
Cohesion: 0.40
Nodes (3): GHOST_PATTERNS, ROOT, TEMPLATES

### Community 129 - "paraglide-keys.test.ts"
Cohesion: 0.50
Nodes (4): ROOT, TEMPLATE_ROOTS, usedKeys(), walk()

### Community 130 - "repository-metadata.test.ts"
Cohesion: 0.40
Nodes (4): packageDirectories, PACKAGES_DIR, ROOT, rootLicense

### Community 131 - "skeleton-inventory.ts"
Cohesion: 0.40
Nodes (4): SKELETON_PRIMITIVES, SKELETON_UTILITIES, SKELETON_UTILITY_PREFIXES, SKELETON_VERSIONS

### Community 133 - "uuid-id-contracts.test.ts"
Cohesion: 0.40
Nodes (3): ID_CONTRACT_PATTERNS, MODULE_TEMPLATES, ROOT

### Community 134 - "TDD Workflow — Red → Green → Refactor"
Cohesion: 0.50
Nodes (4): 1. Red — Write a failing test first, 2. Green — Implement the smallest change, 3. Refactor — Clean up while tests stay green, TDD Workflow — Red → Green → Refactor

### Community 135 - "Contribution Workflow"
Cohesion: 0.50
Nodes (4): Branches, Contribution Workflow, Issues, Pull requests

### Community 137 - "Modules"
Cohesion: 0.50
Nodes (4): Application features, Foundations, Modules, UI and editing

### Community 141 - "tiptap-xss.test.ts"
Cohesion: 0.50
Nodes (3): PREVIEW_FILE, RENDERER_FILE, ROOT

### Community 142 - "tests/upload-security.test.ts"
Cohesion: 0.50
Nodes (3): ROOT, S3_CLIENT, UPLOAD_ENDPOINT

### Community 151 - "Templates"
Cohesion: 0.67
Nodes (3): `base`, `dashboard`, Templates

### Community 159 - "dashboard/package.json"
Cohesion: 0.40
Nodes (4): name, private, type, version

### Community 164 - "tsconfig.test-d.json"
Cohesion: 0.40
Nodes (4): tests/**/*.test-d.ts, ./tsconfig.json, extends, include

### Community 203 - "environment.ts"
Cohesion: 0.50
Nodes (3): building, dev, version

### Community 205 - "svelte-shims.d.ts"
Cohesion: 0.50
Nodes (3): *.client.js, *.server.js, *.svelte

### Community 234 - "admin-users.test.ts"
Cohesion: 0.27
Nodes (5): createCredentialUser(), CreateCredentialUserInput, DuplicateEmailError, ADMIN, auth

## Knowledge Gaps
- **1574 isolated node(s):** `$schema`, `$id`, `title`, `type`, `schemaVersion` (+1569 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 1808 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **103 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `unreadCount()` connect `chat/index.ts` to `Navbar.svelte`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `path` connect `prebuild-utils.ts` to `svforge/scripts/prebuild.ts`, `svforge.test.ts`, `test-local.ts`, `changelog`?**
  _High betweenness centrality (0.015) - this node is a cross-community bridge._
- **Why does `changelog` connect `changelog` to `properties`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
- **What connects `$schema`, `$id`, `title` to the rest of the system?**
  _1574 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `en.json` be split into smaller, more focused modules?**
  _Cohesion score 0.011363636363636364 - nodes in this community are weakly interconnected._
- **Should `fr.json` be split into smaller, more focused modules?**
  _Cohesion score 0.011363636363636364 - nodes in this community are weakly interconnected._
- **Should `design-system.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.08253968253968254 - nodes in this community are weakly interconnected._