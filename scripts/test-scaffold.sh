...
if [ "$TEMPLATE" = "dashboard" ]; then
	...
	CLEANUP_SQL="DELETE FROM \"user\" WHERE email LIKE '%@sf-test.example' OR email = 'attacker@example.com'"
	bun -e 'const { default: postgres } = await import("postgres"); const sql = postgres(process.env.TEST_DATABASE_URL, { max: 1 }); await sql.unsafe(process.env.SF_CLEANUP_SQL); const rows = await sql`SELECT count(*)::int AS n FROM "user"`; if (rows[0].n !== 0) { console.error("leftover users:", rows[0].n); await sql.end(); process.exit(1); } await sql.end();' \
		SF_CLEANUP_SQL="$CLEANUP_SQL" \
