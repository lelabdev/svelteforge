...
├── src/lib/server/                  ← auth (better-auth), db (drizzle/sqlite), schemas (zod), admin.ts
...
- `dashboard.ts` : deps runtime (drizzle-orm, better-auth, @libsql/client) + devDeps (drizzle-kit, vitest, playwright si opt-in) + patch `package.json` (scripts test) + `baseFiles` puis overlay `dashboardFiles` (filtrage playwright)
