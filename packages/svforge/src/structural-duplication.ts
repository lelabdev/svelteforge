...
import { SKELETON_PRIMITIVES } from './skeleton-inventory';
 ...
 * hide a copied SVForge component. The cache is process-local: `check` and
 * editor integrations can reuse fingerprints without writing project files.
 */
...
const fingerprintCache = new Map<string, Fingerprint>();

function staticAttributeValue(attribute: any): string {
	if (!Array.isArray(attribute.value)) return '';
	return attribute.value.filter((value: any) => value.type === 'Text').map((value: any) => value.data).join(' ');
}
...
export function fingerprintSvelte(source: string, cacheKey?: string): Fingerprint {
	if (cacheKey && fingerprintCache.has(cacheKey)) return fingerprintCache.get(cacheKey)!;
	const fingerprint: Fingerprint = { elements: [], utilities: [], props: [], composition: [], sequence: [] };
	try {
		const ast = parse(source) as any;
		const visit = (node: any) => {
			if (!node || typeof node !== 'object') return;
			if (node.type === 'Element' || node.type === 'Component' || node.type === 'SvelteComponent') {
				const name = typeof node.name === 'string' ? node.name : 'dynamic';
				const token = `${node.type === 'Element' ? 'element' : 'component'}:${name}`;
				fingerprint.elements.push(token);
				fingerprint.sequence.push(token);
				for (const attribute of node.attributes ?? []) {
					if (attribute.type !== 'Attribute') continue;
					fingerprint.props.push(`${token}:${attribute.name}`);
					if (attribute.name === 'class') {
						for (const utility of staticAttributeValue(attribute).split(/\s+/).filter(Boolean)) {
							// Only design-system utilities participate: ordinary layout
							// variation must not make two components look identical.
			...
			for (const [key, value] of Object.entries(node)) {
				...
				else if (value && typeof value === 'object' && (value as any).type) visit(value);
			}
		};
		visit(ast.html);
	} catch {
...
function similarity(left: Fingerprint, right: Fingerprint): number {
	return (
		overlap(left.elements, right.elements) * 0.35 +
		overlap(left.utilities, right.utilities) * 0.2 +
		overlap(left.props, right.props) * 0.2 +
		overlap(left.composition, right.composition) * 0.1 +
		sequenceSimilarity(left.sequence, right.sequence) * 0.15
	);
}
...
/** Compare local components against fingerprints derived from the catalog. */
export function checkStructuralDuplicates(projectRoot: string): StructuralDuplicate[] {
	...
	const references = Object.entries(SVFORGE_CATALOG)
		.map(([component, entry]) => {
	...
	for (const file of svelteFiles(componentsRoot)) {
		...
		// Skeleton's generated inventory remains the authority for name-based
		// primitive duplication; structural matching supplements SVForge reuse.
		if (SKELETON_PRIMITIVES.includes(path.basename(file, '.svelte'))) continue;
		...
		for (const reference of references) {
			...
			findings.push({
				file: path.relative(projectRoot, file), component: reference.component, severity: 'warn', score, evidence
			});
		}
	}
